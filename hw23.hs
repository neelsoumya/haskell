...
{-
dfs [y] l = dfs (findVertices y l) l
-}

{-
dfs (y:ys) l = ((concat (dfs y l)) ++ (concat (dfs ys l))) ++ [y]
-}


{-
dfs (y:ys) l = [y] ++ (dfs [y] l) ++ (dfs ys l)
-}

dfs (y:ys) l = [y] ++ (dfs (findVertices y l) l) ++ (dfs ys l)

{-
dfs v l = (flatten (dfs x l)) ++ (flatten (dfs xs l)) 
		where (x:xs) = findVertices v l
-}
